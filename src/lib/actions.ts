...
// ── Calling Module Actions ──────────────────────────────────────────────────

/** Assign the next highest-priority lead to the current user */
export async function assignNextCallingLead(): Promise<void> {
  "use server";
  const me = await requireAuth();
  const now = new Date();

  // Release any stale assignment older than 2 hours (safety valve)
  await prisma.callingLead.updateMany({
    where: {
      assignedToId: me.id,
      queueStatus: "ASSIGNED",
      assignedAt: { lt: new Date(now.getTime() - 2 * 60 * 60 * 1000) },
    },
    data: { assignedToId: null, assignedAt: null, queueStatus: "PENDING" },
  });

  // Already has one? do nothing
  const current = await prisma.callingLead.findFirst({
    where: { assignedToId: me.id, queueStatus: "ASSIGNED" },
  });
  if (current) { revalidatePath("/calling/queue"); return; }

  // Priority order: follow-ups due → no-answer retries due → priority 1 → 2 → 3 → 4 → 5
  const candidateWhere = [
    { queueStatus: "FOLLOW_UP",        followUpAt:    { lte: now }, assignedToId: null },
    { queueStatus: "NO_ANSWER_RETRY",  nextRetryAt:   { lte: now }, assignedToId: null },
    { queueStatus: "PENDING",          priority: 1,                 assignedToId: null },
    { queueStatus: "PENDING",          priority: 2,                 assignedToId: null },
    { queueStatus: "PENDING",          priority: 3,                 assignedToId: null },
    { queueStatus: "PENDING",          priority: 4,                 assignedToId: null },
    { queueStatus: "PENDING",          priority: 5,                 assignedToId: null },
  ];

  for (const where of candidateWhere) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const lead = await prisma.callingLead.findFirst({
      where: where as any,
      orderBy: { createdAt: "asc" },
    });
    if (lead) {
      await prisma.callingLead.update({
        where: { id: lead.id },
        data: { assignedToId: me.id, assignedAt: now, queueStatus: "ASSIGNED" },
      });
      break;
    }
  }

  revalidatePath("/calling/queue");
}

/** Log a call result and update the lead's queue state */
export async function logCallingResult(
  leadId: number,
  status: string,
  note?: string,
  followUpAt?: string
  ...
  const lead = await prisma.callingLead.findUnique({ where: { id: leadId } });
  if (!lead) throw new Error("Lead not found");

  let newQueueStatus = "PENDING";
  const updateData: Record<string, unknown> = { assignedToId: null, assignedAt: null };

  if (status === "ORDER_CONFIRMED" || status === "NOT_INTERESTED" || status === "WRONG_NUMBER") {
    newQueueStatus = "DONE";
  } else if (status === "NO_ANSWER" || status === "BUSY") {
    const count = lead.noAnswerCount + 1;
    updateData.noAnswerCount = count;
    if (count >= 3) {
      newQueueStatus = "UNREACHABLE";
    } else {
      newQueueStatus = "NO_ANSWER_RETRY";
      // 1st retry: +2 hours; 2nd retry: next morning 9am PKT
      if (count === 1) {
        updateData.nextRetryAt = new Date(now.getTime() + 2 * 60 * 60 * 1000);
      } else {
        const tomorrow = new Date(`${new Date(now.getTime() + 24 * 60 * 60 * 1000)
          .toLocaleDateString("en-CA", { timeZone: "Asia/Karachi" })}T09:00:00+05:00`);
        updateData.nextRetryAt = tomorrow;
      }
    }
  } else if (status === "INTERESTED" || status === "FOLLOW_UP_REQUIRED") {
    newQueueStatus = "FOLLOW_UP";
  ...
  await prisma.$transaction([
    prisma.callingLead.update({
      where: { id: leadId },
      data: { ...updateData, queueStatus: newQueueStatus } as Parameters<typeof prisma.callingLead.update>[0]["data"],
    }),
    prisma.callingRecord.create({
      data: {
        leadId,
        calledById: me.id,
        status,
  ...
  revalidatePath("/calling/queue");
  revalidatePath("/calling/dashboard");
}

/** Release current assignment (admin override or employee giving up) */
export async function releaseCallingLead(leadId: number): Promise<void> {
  "use server";
  await requireAuth();
  await prisma.callingLead.update({
    where: { id: leadId },
...
/** Clone leads from ReorderLead into CallingLead (skips existing) */
export async function importCallingLeadsFromReorder(): Promise<number> {
  "use server";
  const me = await requireAuth();
  if (!me.isAdmin) throw new Error("Unauthorized");

  const reorderLeads = await prisma.reorderLead.findMany({
    where: { status: { notIn: ["ORDER_RECEIVED", "NOT_INTERESTED", "CANCELLED"] } },
  });

  let imported = 0;
  for (const rl of reorderLeads) {
    const ref = `reorder:${rl.id}`;
    const exists = await prisma.callingLead.findFirst({ where: { sourceRef: ref } });
    if (exists) continue;
    ...
    if (rl.status === "NO_ANSWER") {
      queueStatus = "NO_ANSWER_RETRY";
      noAnswerCount = 1;
      nextRetryAt = new Date(Date.now() + 2 * 60 * 60 * 1000);
      priority = 3;
    } else if (rl.status === "ORDER_PLACED" || rl.status === "INTERESTED_LATER" || rl.status === "CALLBACK") {
      queueStatus = "FOLLOW_UP";
      priority = 2;
      followUpAt = rl.followUpDate ? new Date(rl.followUpDate) : new Date(Date.now() + 24 * 60 * 60 * 1000);
    }

    await prisma.callingLead.create({
      data: {
        customerName: rl.customerName,
        phone: rl.phone,
        city: rl.city || null,
        prevItem: rl.prevItem || null,
        sourceType: "REORDER",
        sourceRef: ref,
        queueStatus,
        priority,
...
/** Add calling leads from retail customers (15+ days since last order) */
export async function importCallingLeadsFromRetail(): Promise<number> {
  "use server";
  const me = await requireAuth();
  if (!me.isAdmin) throw new Error("Unauthorized");
  ...
  const orders = await prisma.retailOrder.findMany({
    where: { date: { lte: cutoff } },
    orderBy: { date: "desc" },
    select: { customerName: true, phone: true, date: true, id: true },
  });

  // Dedupe by phone, keep latest order
