...
async function AdsContent({ from, to }: { from: string; to: string }) {
  ...
  return (
    <>
      ...
      <Section dot="bg-emerald-500" title="Verified — from your website orders">
      ...
      <Section dot="bg-[#1877F2]" title="Meta-reported — estimated by Meta's own attribution">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <StatCard label="Revenue (Meta-reported)" value={`Rs ${fmt(meta.reportedRevenue)}`} accent="bg-[#1877F2]/40" valueClass="text-gray-500" />
          <StatCard label="ROAS (Meta-reported)" value={`${meta.reportedRoas.toFixed(2)}x`} accent="bg-[#1877F2]/40" valueClass="text-gray-500" />
        </div>
        <p className="text-xs text-gray-400 mt-2">
