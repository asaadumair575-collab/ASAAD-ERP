...
import { updateModuleAccess, deleteUserAssign } from "@/lib/actions";
    ...
    <div className="max-w-3xl space-y-6 pb-10">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <Link href="/settings/user-assign" className="text-gray-400 hover:text-black transition-colors shrink-0">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-semibold tracking-tight truncate">{user.displayName ?? user.username}</h1>
        ...
        <DeleteButton
          action={deleteUserAssign.bind(null, userId)}
          label="Delete User"
          message={`Permanently delete ${user.displayName ?? user.username}? This cannot be undone.`}
        />
      </div>
