  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link
          href="/leads/new"
          className="shrink-0 flex items-center gap-1.5 bg-black text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors"
        >
      ...
      <form method="GET" className="flex flex-wrap gap-2 items-center">
        ...
        {(q || status || city) && (
          <Link href="/leads" className="text-sm text-gray-500 hover:text-black transition-colors">
            Clear
          </Link>
        )}
      </form>
      ...
      {(q || status || city) && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {status && (
            <Link href={{ pathname: "/leads", query: { q, city } }} className="filter-chip">
              {STATUS_LABELS[status] ?? status}<span className="filter-chip-x">×</span>
            </Link>
          )}
          {city && (
            <Link href={{ pathname: "/leads", query: { q, status } }} className="filter-chip">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-gray-500">
          ...
          <div className="flex gap-2">
            {currentPage > 1 && (
              <Link
                href={{ pathname: "/leads", query: { q, status, city, page: currentPage - 1 } }}
