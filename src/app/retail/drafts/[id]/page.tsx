    ...
    <div className="max-w-sm mx-auto space-y-4">
      ...
      <div className="relative bg-white border border-gray-300 rounded-xl overflow-hidden shadow-md print:shadow-none print:border-gray-400">
        ...
        <div className="px-5 py-2 border-b border-dashed border-gray-300 bg-gray-50 flex items-center justify-between">
          {draft.confirmed ? (
            <span className="inline-flex items-center bg-green-50 border border-green-300 text-green-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide">
              Confirmed
            </span>
          ) : (
            <span className="inline-flex items-center bg-red-50 border border-red-300 text-red-600 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide">
              Not Confirmed
            </span>
          )}
          {draft.confirmed ? (
          ...
          <div className="flex divide-x divide-gray-200 px-2 pb-3">
            ...
            <div className="flex-1 flex flex-col items-center gap-1 px-3 py-1 text-center">
              ...
              {profile?.bankAccountNumber ? (
                <p className="text-sm font-black text-gray-500 tracking-wide break-all">{profile.bankAccountNumber}</p>
              ) : (
                <p className="text-[9px] text-red-400">Not configured</p>
              )}
              {profile?.bankAccountTitle && (
                <p className="text-[10px] font-extrabold text-gray-600">{profile.bankAccountTitle}</p>
              )}
            </div>
          </div>
        </div>
