...
export async function GET(req: Request) {
  ...
  for (const t of tokens) {
    ...
    for (const hs of headerStyles) {
      for (const v of ["v3", "v1"]) {
        const url = `${BASE}/${v}/get-track-order/${cn}`;
        try {
          const res = await fetch(url, { headers: hs.headers, cache: "no-store", signal: AbortSignal.timeout(8000) });
          const body = (await res.text()).slice(0, 200);
          results.push({ token: t.name, auth: hs.auth, url: `${v}`, status: res.status, body });
          if (res.ok) break;
        } catch (e) {
          results.push({ token: t.name, auth: hs.auth, url: `${v}`, status: "err", body: String(e).slice(0, 100) });
        }
      }
    }
  }

  return NextResponse.json({ cn, tokensConfigured: tokens.map((t) => t.name), results });
}
